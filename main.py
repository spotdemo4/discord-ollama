...
async def on_ask(interaction, question: str):
    ...
    if chat_response:
        await interaction.followup.send(chat_response)
    else:
...
async def on_chat(interaction):
    ...
    await interaction.response.send_message("Chat started!", ephemeral=True)
...
async def on_message(message):
    ...
    if message.channel.name == f"Chat with {client.user.display_name}":
        ...
        if chat_response:
            await message.channel.send(content=chat_response)
        else:
            await message.channel.send(content=f"Could not process message: {message.content}")
